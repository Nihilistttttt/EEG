...
#define NUM_CHANNELS 8                                             // 当前主要分析 CP3、CP4、C3、C4 四个脑电通道。
...
typedef struct {
    ...
    float CH4Mags[FFT_SIZE / 2 + 1];
    float CH5Mags[FFT_SIZE / 2 + 1];
    float CH6Mags[FFT_SIZE / 2 + 1];
    float CH7Mags[FFT_SIZE / 2 + 1];
...
static float theta_pow_ch4, alpha_pow_ch4, beta_pow_ch4;
static float theta_pow_ch5, alpha_pow_ch5, beta_pow_ch5;
static float theta_pow_ch6, alpha_pow_ch6, beta_pow_ch6;
static float theta_pow_ch7, alpha_pow_ch7, beta_pow_ch7;
...
IIR_SOS_t Bandpass_CH4 = {
    .Sec = {
            {.b0 = 0.93637848f, .b1 = -1.87275696f, .b2 = 0.93637848f, .a1 = -1.95979169f, .a2 = 0.96227012f, .w1 = 0, .w2 = 0},
            {.b0 = 1.0f, .b1 = -2.0f, .b2 = 1.0f, .a1 = -1.90886498f, .a2 = 0.91127901f, .w1 = 0, .w2 = 0}},
    .NumSections = 2
};
IIR_SOS_t Bandpass_CH5 = {
    .Sec = {
            {.b0 = 0.93637848f, .b1 = -1.87275696f, .b2 = 0.93637848f, .a1 = -1.95979169f, .a2 = 0.96227012f, .w1 = 0, .w2 = 0},
            {.b0 = 1.0f, .b1 = -2.0f, .b2 = 1.0f, .a1 = -1.90886498f, .a2 = 0.91127901f, .w1 = 0, .w2 = 0}},
    .NumSections = 2
};
IIR_SOS_t Bandpass_CH6 = {
    .Sec = {
            {.b0 = 0.93637848f, .b1 = -1.87275696f, .b2 = 0.93637848f, .a1 = -1.95979169f, .a2 = 0.96227012f, .w1 = 0, .w2 = 0},
            {.b0 = 1.0f, .b1 = -2.0f, .b2 = 1.0f, .a1 = -1.90886498f, .a2 = 0.91127901f, .w1 = 0, .w2 = 0}},
    .NumSections = 2
};
IIR_SOS_t Bandpass_CH7 = {
    .Sec = {
            {.b0 = 0.93637848f, .b1 = -1.87275696f, .b2 = 0.93637848f, .a1 = -1.95979169f, .a2 = 0.96227012f, .w1 = 0, .w2 = 0},
            {.b0 = 1.0f, .b1 = -2.0f, .b2 = 1.0f, .a1 = -1.90886498f, .a2 = 0.91127901f, .w1 = 0, .w2 = 0}},
    .NumSections = 2
 ...
 * 只用于显示，不参与训练和 FFT。
 */
static float wave_display_base_ch[8];
...
static EEG_DriftRemove_t AB_Drift_CH4 = {0.0f, 0};
static EEG_DriftRemove_t AB_Drift_CH5 = {0.0f, 0};
static EEG_DriftRemove_t AB_Drift_CH6 = {0.0f, 0};
static EEG_DriftRemove_t AB_Drift_CH7 = {0.0f, 0};
...
static void Direction_BuildRawFeature(float feature[DIR_FEAT_DIM_LOCAL])
{
    float theta_mi[4] = {
        ...
        Direction_SafePower(theta_pow_ch4), Direction_SafePower(theta_pow_ch5)
    };
    float mu_mi[4] = {
        Direction_SafePower(alpha_pow_ch2), Direction_SafePower(alpha_pow_ch3),
        Direction_SafePower(alpha_pow_ch4), Direction_SafePower(alpha_pow_ch5)
    };
    float beta_mi[4] = {
        Direction_SafePower(beta_pow_ch2), Direction_SafePower(beta_pow_ch3),
        Direction_SafePower(beta_pow_ch4), Direction_SafePower(beta_pow_ch5)
    };
    float total_mi[4];

    for (int i = 0; i < 4; i++) {
        total_mi[i] = Direction_SafePower(theta_mi[i] + mu_mi[i] + beta_mi[i]);
    }

    feature[0] = Direction_LogRatio(mu_mi[0],    mu_mi[1])    * DIR_FEATURE_SCALE;
    feature[1] = Direction_LogRatio(beta_mi[0],  beta_mi[1])  * DIR_FEATURE_SCALE;
    feature[2] = Direction_LogRatio(mu_mi[2],    mu_mi[3])    * DIR_FEATURE_SCALE;
    feature[3] = Direction_LogRatio(beta_mi[2],  beta_mi[3])  * DIR_FEATURE_SCALE;
    feature[4] = Direction_LogRatio(theta_mi[0], theta_mi[1]) * DIR_FEATURE_SCALE;
    feature[5] = Direction_LogRatio(theta_mi[2], theta_mi[3]) * DIR_FEATURE_SCALE;
    feature[6] = Direction_LogRatio(total_mi[0], total_mi[1]) * DIR_FEATURE_SCALE;
    feature[7] = Direction_LogRatio(total_mi[2], total_mi[3]) * DIR_FEATURE_SCALE;

    feature[8]  = Direction_LogRatio(mu_mi[0],   total_mi[0]) * DIR_FEATURE_SCALE;
    feature[9]  = Direction_LogRatio(mu_mi[1],   total_mi[1]) * DIR_FEATURE_SCALE;
    feature[10] = Direction_LogRatio(mu_mi[2],   total_mi[2]) * DIR_FEATURE_SCALE;
    feature[11] = Direction_LogRatio(mu_mi[3],   total_mi[3]) * DIR_FEATURE_SCALE;
    feature[12] = Direction_LogRatio(beta_mi[0], total_mi[0]) * DIR_FEATURE_SCALE;
    feature[13] = Direction_LogRatio(beta_mi[1], total_mi[1]) * DIR_FEATURE_SCALE;
    feature[14] = Direction_LogRatio(beta_mi[2], total_mi[2]) * DIR_FEATURE_SCALE;
    feature[15] = Direction_LogRatio(beta_mi[3], total_mi[3]) * DIR_FEATURE_SCALE;
    feature[16] = Direction_LogRatio(beta_mi[0], mu_mi[0])    * DIR_FEATURE_SCALE;
    feature[17] = Direction_LogRatio(beta_mi[1], mu_mi[1])    * DIR_FEATURE_SCALE;
    feature[18] = Direction_LogRatio(beta_mi[2], mu_mi[2])    * DIR_FEATURE_SCALE;
    feature[19] = Direction_LogRatio(beta_mi[3], mu_mi[3])    * DIR_FEATURE_SCALE;

    feature[20] = Direction_LogRatio(mu_mi[0],   mu_mi[2])    * DIR_FEATURE_SCALE;
    feature[21] = Direction_LogRatio(mu_mi[1],   mu_mi[3])    * DIR_FEATURE_SCALE;
    feature[22] = Direction_LogRatio(beta_mi[0], beta_mi[2])  * DIR_FEATURE_SCALE;
    feature[23] = Direction_LogRatio(beta_mi[1], beta_mi[3])  * DIR_FEATURE_SCALE;
}
...
static void Waveform_RemoveDisplayBaseline8ch(const float in[8], float out[8])
{
#if WAVE_DISPLAY_REMOVE_BASELINE
    if (!wave_display_base_ready) {
        for (int i = 0; i < 8; i++) wave_display_base_ch[i] = in[i];
        wave_display_base_ready = 1;
    }
    for (int i = 0; i < 8; i++) {
        wave_display_base_ch[i] += WAVE_DISPLAY_BASELINE_ALPHA * (in[i] - wave_display_base_ch[i]);
        out[i] = in[i] - wave_display_base_ch[i];
    }
#else
...
static void CSP_PrintWindowCSV(uint8_t label)
{
    ...
    for (j = 0; j < CSP_WIN_SIZE; j++) {
        ...
        int32_t ch0 = CSP_VoltToUVX1000(RingBufFiltered.CH2[idx]);
        int32_t ch1 = CSP_VoltToUVX1000(RingBufFiltered.CH3[idx]);
        int32_t ch2 = CSP_VoltToUVX1000(RingBufFiltered.CH4[idx]);
        int32_t ch3 = CSP_VoltToUVX1000(RingBufFiltered.CH5[idx]);
...
static uint8_t Process_FFT_Step (void) {
    ...
    case FFT_STEP_FREQ_FILTER:
        ...
        raw_send_channel = 0;
        current_send_frag = 0;
        fft_step = FFT_STEP_SEND_RAW_SPECTRUM;
        break;

    case FFT_STEP_SEND_RAW_SPECTRUM: {
        float *mags = FFT_Data_GetMags(&FFT_Data, raw_send_channel);
        CmdType cmd = (CmdType)(CMD_RAW_SPECTRUM_CH0 + raw_send_channel);
        Send_Spectrum (cmd, mags, current_send_frag);
        current_send_frag++;
        if (current_send_frag >= SPECTRUM_TOTAL_FRAGS) {
            current_send_frag = 0;
            raw_send_channel++;
            if (raw_send_channel >= NUM_CHANNELS) {
                raw_send_channel = 0;
    ...
    case FFT_STEP_BAND_POWER: {
        BandPowers_t bp[8];
        for (int i = 0; i < 8; i++) {
            bp[i] = compute_band_powers(FFT_Data_GetMags(&FFT_Data, i), FFT_SIZE,
        ...
        theta_pow_ch0 = bp[0].theta_power; alpha_pow_ch0 = bp[0].alpha_power; beta_pow_ch0 = bp[0].beta_power;
        theta_pow_ch1 = bp[1].theta_power; alpha_pow_ch1 = bp[1].alpha_power; beta_pow_ch1 = bp[1].beta_power;
        theta_pow_ch2 = bp[2].theta_power; alpha_pow_ch2 = bp[2].alpha_power; beta_pow_ch2 = bp[2].beta_power;
        theta_pow_ch3 = bp[3].theta_power; alpha_pow_ch3 = bp[3].alpha_power; beta_pow_ch3 = bp[3].beta_power;
        theta_pow_ch4 = bp[4].theta_power; alpha_pow_ch4 = bp[4].alpha_power; beta_pow_ch4 = bp[4].beta_power;
        theta_pow_ch5 = bp[5].theta_power; alpha_pow_ch5 = bp[5].alpha_power; beta_pow_ch5 = bp[5].beta_power;
        theta_pow_ch6 = bp[6].theta_power; alpha_pow_ch6 = bp[6].alpha_power; beta_pow_ch6 = bp[6].beta_power;
        theta_pow_ch7 = bp[7].theta_power; alpha_pow_ch7 = bp[7].alpha_power; beta_pow_ch7 = bp[7].beta_power;
    ...
    /* ----------           ---------- */
    case FFT_STEP_ATTENTION:
        cur_attn0  = compute_attention_feature(theta_pow_ch0, alpha_pow_ch0, beta_pow_ch0);  /* F3 */
        cur_attn1  = compute_attention_feature(theta_pow_ch1, alpha_pow_ch1, beta_pow_ch1);  /* F4 */
        cur_relax0 = compute_relaxation_feature(theta_pow_ch0, alpha_pow_ch0, beta_pow_ch0);
        cur_relax1 = compute_relaxation_feature(theta_pow_ch1, alpha_pow_ch1, beta_pow_ch1);
...
case FFT_STEP_UPDATE_TREND:
    ...
    case FFT_STEP_SEND_FILT_SPECTRUM: {
        float *mags = FFT_Data_GetMags(&FFT_DataFiltered, filt_send_channel);
        CmdType cmd = (CmdType)(CMD_FILT_SPECTRUM_CH0 + filt_send_channel);
        Send_Spectrum (cmd, mags, current_filt_send_frag);
        current_filt_send_frag++;
        if (current_filt_send_frag >= SPECTRUM_TOTAL_FRAGS) {
            current_filt_send_frag = 0;
            filt_send_channel++;
            if (filt_send_channel >= NUM_CHANNELS) {
                filt_send_channel = 0;
...
void Signal_Analysis_Start (void) {
    ...
    // W25Q64_Init();   // 如需保存到外部 Flash，可启用。
    // ADS1299_Init();  // 已在上方初始化，这里不需要重复调用。
    /* ---------- 2. 初始化 256 点复数 FFT ---------- */
    ...
    if (ring_buffer_get_frame (frame_buf)) {
        ...
        /* 解析 ADS1299 完整 8 通道帧：CH0=F3, CH1=F4, CH2=CP3, CH3=CP4, CH4=C3, CH5=C4, CH6=P3, CH7=P4。 */
        ADS1299_ParseRawFrame (frame_buf, NULL, ch_data);

        float val_ch[8];
        val_ch[0] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_F3],  4.5f, 24.0f);
        val_ch[1] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_F4],  4.5f, 24.0f);
        val_ch[2] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_CP3], 4.5f, 24.0f);
        val_ch[3] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_CP4], 4.5f, 24.0f);
        val_ch[4] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_C3],  4.5f, 24.0f);
        val_ch[5] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_C4],  4.5f, 24.0f);
        val_ch[6] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_P3],  4.5f, 24.0f);
        val_ch[7] = ADS1299_CodeToVolt (ch_data[ADS1299_EEG_CH_P4],  4.5f, 24.0f);

        float show_ch[8];
        Waveform_RemoveDisplayBaseline8ch(val_ch, show_ch);
        #if !DIR_1S_TEST_TEXT_ONLY
        Send_Waveform8ch (CMD_RAW_WAVE_8CH, show_ch);
        #endif

        /* ========== 八通道实时去电极慢漂移 ========== */
        EEG_DriftRemove_t *drift_ptr[8] = {&AB_Drift_CH0, &AB_Drift_CH1, &AB_Drift_CH2, &AB_Drift_CH3,
                                            &AB_Drift_CH4, &AB_Drift_CH5, &AB_Drift_CH6, &AB_Drift_CH7};
        float drift_ch[8];
        for (int i = 0; i < 8; i++) {
            drift_ch[i] = AB_RemoveRealtimeDrift(val_ch[i], drift_ptr[i]);
        }

        /* ========== 八通道 50Hz 陷波 ========== */
        IIR_SOS_t *notch_ptr[8] = {&Notch_CH0, &Notch_CH1, &Notch_CH2, &Notch_CH3,
                                    &Notch_CH4, &Notch_CH5, &Notch_CH6, &Notch_CH7};
        float ab_pre_ch[8];
#if AB_REALTIME_NOTCH_ENABLE
        for (int i = 0; i < 8; i++) {
            ab_pre_ch[i] = IIR_SOS_Step(drift_ch[i], notch_ptr[i]);
        }
#else
        for (int i = 0; i < 8; i++) {
            ab_pre_ch[i] = drift_ch[i];
        }
#endif

        /* 八通道带通滤波 */
        IIR_SOS_t *bp_ptr[8] = {&Bandpass_CH0, &Bandpass_CH1, &Bandpass_CH2, &Bandpass_CH3,
                                 &Bandpass_CH4, &Bandpass_CH5, &Bandpass_CH6, &Bandpass_CH7};
        float filtered_ch[8];
        for (int i = 0; i < 8; i++) {
            filtered_ch[i] = IIR_SOS_Step(ab_pre_ch[i], bp_ptr[i]);
        }

        RingBufFiltered.CH0[RingBufFiltered.WriteIdx] = filtered_ch[0];
        RingBufFiltered.CH1[RingBufFiltered.WriteIdx] = filtered_ch[1];
        RingBufFiltered.CH2[RingBufFiltered.WriteIdx] = filtered_ch[2];
        RingBufFiltered.CH3[RingBufFiltered.WriteIdx] = filtered_ch[3];
        RingBufFiltered.CH4[RingBufFiltered.WriteIdx] = filtered_ch[4];
        RingBufFiltered.CH5[RingBufFiltered.WriteIdx] = filtered_ch[5];
        RingBufFiltered.CH6[RingBufFiltered.WriteIdx] = filtered_ch[6];
        RingBufFiltered.CH7[RingBufFiltered.WriteIdx] = filtered_ch[7];
        ...
        #if !DIR_1S_TEST_TEXT_ONLY
        Send_Waveform8ch (CMD_FILT_WAVE_8CH, filtered_ch);
        #endif

        /* ========== 八通道写入 FFT 环形缓冲区 ========== */
        RingBuf.CH0[RingBuf.WriteIdx] = ab_pre_ch[0];
        RingBuf.CH1[RingBuf.WriteIdx] = ab_pre_ch[1];
        RingBuf.CH2[RingBuf.WriteIdx] = ab_pre_ch[2];
        RingBuf.CH3[RingBuf.WriteIdx] = ab_pre_ch[3];
        RingBuf.CH4[RingBuf.WriteIdx] = ab_pre_ch[4];
        RingBuf.CH5[RingBuf.WriteIdx] = ab_pre_ch[5];
        RingBuf.CH6[RingBuf.WriteIdx] = ab_pre_ch[6];
        RingBuf.CH7[RingBuf.WriteIdx] = ab_pre_ch[7];
        RingBuf.WriteIdx = (RingBuf.WriteIdx + 1) % FFT_SIZE;

        //      OLED     
        Update_Waveform (show_ch[0]);

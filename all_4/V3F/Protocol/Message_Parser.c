...
void Send_Waveform8ch(CmdType wave_type, const float vals[8]) {
    uint8_t payload[32];
    for (int i = 0; i < 8; i++) {
        memcpy(payload + i * 4, &vals[i], 4);
    }
    if (wave_type == CMD_RAW_WAVE_8CH || wave_type == CMD_FILT_WAVE_8CH) {
        Pack_Frame(SERIAL_PORT_WIFI, wave_type, payload, 32);
    }
}
...
void Send_Spectrum(CmdType spectrum_type, float *mag, uint8_t frag_idx) {
    ...
    if (spectrum_type >= CMD_RAW_SPECTRUM_CH0 && spectrum_type <= CMD_RAW_SPECTRUM_CH7) {
        Pack_Frame_Fragmented(SERIAL_PORT_WIFI, spectrum_type, payload, FRAG_SIZE,
                              frag_idx, TOTAL_FRAGS);
    } else if (spectrum_type >= CMD_FILT_SPECTRUM_CH0 && spectrum_type <= CMD_FILT_SPECTRUM_CH7) {
        Pack_Frame_Fragmented(SERIAL_PORT_WIFI, spectrum_type, payload, FRAG_SIZE,
                              frag_idx, TOTAL_FRAGS);
    }
    // 如需延时，可取消注释

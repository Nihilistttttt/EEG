...
public class TrainingFragment extends Fragment implements DoctorConnector.DataListener {
    ...
    private void updateRestTimer() {
        ...
        if (elapsed >= REST_DURATION_MS) enterTrialPhase();
        else handler.postDelayed(this::updateRestTimer, 100);
    }
    ...
    private void updateTrialTimer() {
        ...
        if (elapsed >= TRIAL_DURATION_MS) {
            currentTrial++;
            if (currentTrial >= totalTrials) startModelTraining();
            else enterRestPhase();
        } else handler.postDelayed(this::updateTrialTimer, 100);
    }
    ...
    @Override public void onDestroyView() { super.onDestroyView(); handler.removeCallbacksAndMessages(null); }

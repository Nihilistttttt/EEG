...
# ========================= 训练参数（仅改变采集组织，不改变界面布局） =========================
# 普通24维采集保留原来的左右各2次；CSP增加独立试次数量，降低重叠窗口造成的过拟合。
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dir_row_trial_ids = []   # 与train_data逐行对应，仅用于24维按试次验证
        ...
        self.csp_window_trial_map = {}  # (label, win_id) -> trial_id；CSV格式保持不变，另存元数据
    ...
    def toggle_serial(self):
        ...
        if not port:
            ...
            return
        self.serial_thread = SerialThread(port, SERIAL_BAUD)
        ...
        self.serial_thread.send_command("MODEL,GET")
        self.update_central_status("空闲", "#7f8c8d")
    ...
    def parse_serial_line(self, line):
        ...
        if line.startswith('CSP_END,'):
            if self.csp_receiving:
                ...
                # 记录该窗口属于哪个独立试次。主CSV列格式不变，训练脚本通过旁路元数据准确分组。
        ...
        if line.startswith('MODE_SET_OK,'):
            ...
            # 如果当前有等待的启动命令，先记录；COLLECT/CSP还需完成模型绑定。
            work_command = None
            ...
            # 采集模式与推理模型明确绑定；INFER只切换运行状态，不覆盖已选模型。
            bound_model = {1: "FFT24", 3: "CSP"}.get(mode)
            if bound_model:
                self.pending_work_command = work_command
                self.serial_thread.send_command(f"MODEL,SET,{bound_model}")
                self.append_log(
                ...
                self.pending_work_command = None
                self.serial_thread.send_command(work_command)
        ...
        # ---- POSTURE_STATE 姿态变化事件（新版DEBUG不附带重力向量） ----
    ...
    def gravity_to_posture_angles(gx_mg, gy_mg, gz_mg):
        """由现有重力向量计算显示用横滚/俯仰角；不参与MCU姿态与翻身判断。"""
        gx = float(gx_mg)
        gy = float(gy_mg)
        gz = float(gz_mg)
        ...
        # 重力向量绕自身旋转时不变化；没有磁力计或上传的Z轴角速度，无法观测偏航角。
        yaw_deg = None
    ...
    def start_training(self):
        ...
        # COLLECT保持原有4组；CSP增加独立试次并采用平衡随机顺序。
        if mode == 3:
    ...
    def save_training_data(self):
        if self.train_data:
            ...
            try:
                ...
                # CSV列保持不变；同名JSON只记录逐行试次编号，供训练时分组验证。
            ...
            # CSP表格格式仍为 label,win_id,sample_idx,ch0..ch3，训练脚本可直接读取。
            ...
            try:
                ...
                # 主CSV格式完全保留；旁路JSON只用于训练时准确识别独立试次。
                meta_path = output_path.with_name(output_path.stem + '_meta.json')
                meta = {
                    'csv_file': output_path.name,
                    'trial_sequence': self.trial_sequence,

...
/* ---------- 预计算带通系数 (4阶 Butterworth 0.1~10Hz, 250SPS) ---------- */
...
/* ---------- 内部辅助函数声明 ---------- */
static void sos_filter_step(const real_t *b0, const real_t *b1, const real_t *b2,
...
void blink_detector_finish_block(BlinkDetector_t *bd,
    ...
    real_t min_prom = bd->prominence_k * 1.0f; // 噪声估计简化为 1.0，实际应基于 MAD
    ...
    for (int i = 0; i < np && bd->pending_count < BLINK_MAX_EVENTS; i++) {
        ...
        if (peak_local > confirm_boundary) continue;  // 未确认
...
void blink_detector_get_statistics(const BlinkDetector_t *bd, BlinkStatistics_t *stats) {
    ...
    stats->blink_rate   = (real_t)recent_count;   // 最近窗口总次数 (次/分钟)
...
static void sos_filter_step(const real_t *b0, const real_t *b1, const real_t *b2,
                            ...
                            real_t input, real_t *output) {
    for (int s = 0; s < 2; s++) {
        real_t w = input - a1[s] * w1[s] - a2[s] * w2[s];
        real_t y = b0[s] * w + b1[s] * w1[s] + b2[s] * w2[s];
        w2[s] = w1[s];
        w1[s] = w;
        input = y;
    }
